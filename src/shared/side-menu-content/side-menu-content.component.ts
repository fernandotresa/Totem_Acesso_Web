...
export class SideMenuContentComponent {
	...
	constructor(private platform: Platform,
		...
		this.eventsCtrl.subscribe('userInfo:menu', info => {
			this.displayName = info.name
			this.displayEmail = info.email

			if(info.photo)
				this.displayPic = info.photo
			else 
				this.displayPic = "assets/imgs/100x100.png"			
		});		
	}

	ngOnDestroy() {
		this.eventsCtrl.unsubscribe(SideMenuOptionSelect);
		this.eventsCtrl.unsubscribe('userInfo:menu');
	}
